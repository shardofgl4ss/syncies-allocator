...
{
	if (arena_thread == nullptr) { if (arena_init() != 0) { goto arena_null; } }
...
alloc_corrupt:
	syn_panic("CRITICAL: Heap corruption in allocator superstructures!");
}
...
syn_reset()
{
	...
	if (arena_thread == nullptr) goto arena_null;
	#endif
