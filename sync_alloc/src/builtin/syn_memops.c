...
[[gnu::hot]]
int syn_memset(void *restrict target, const u8 c, const usize bytes)
{
	if (bytes == 0 || !target) {
		return 1;
	}
	...
	if (bytes < ALIGNMENT || simd_capability & SIMD_NONE) {
		syn_memset_scalarset(dest, c, bytes);
		return 0;
	}
	const uintptr_t dest_addr = (uintptr_t)dest;
	constexpr u8 simd_width = (simd_capability & SIMD_AVX)
	...
	if (head) {
		syn_memset_scalarset(dest, c, head);
	}
	...
	switch (simd_capability) {
		case SIMD_AVX | SIMD_SSE:
			...
			if (remaining_bytes >= WIDTH_AVX) {
				avx_chunk = remaining_bytes & ~(WIDTH_AVX - 1);
				syn_memset_avxset(dest + head, c, avx_chunk);
				remaining_bytes -= avx_chunk;
			}
			usize sse_chunk = 0;
			if (remaining_bytes >= WIDTH_SSE) {
				sse_chunk = remaining_bytes & ~(WIDTH_SSE - 1);
				syn_memset_sseset(dest + head + avx_chunk, c, sse_chunk);
			}
			tail = remaining_bytes - sse_chunk;
			if (tail) {
			...
			goto skip_tail;
		case SIMD_AVX:
			syn_memset_avxset(dest + head, c, body);
			break;
		case SIMD_SSE:
			syn_memset_sseset(dest + head, c, body);
			break;
		default:
			syn_memset_scalarset(dest + head, c, body);
			break;
	}
	if (tail) {
		syn_memset_scalarset(dest + head + body, c, tail);
	}
skip_tail:
	return 0;
}
